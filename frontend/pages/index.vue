        ...
        <div class="quiz-container">
          ...
          <div class="choices">
            <button
              ...
              :key="index"
              :class="{ selected: selectedAnswer === index + 1, correct: selectedAnswer === index + 1 && isCorrectAnswer, incorrect: selectedAnswer === index + 1 && !isCorrectAnswer }"
          ...
          <button v-if="feedback" class="next-btn" @click="fetchNextQuiz">次へ</button>
        </div>
      </div>
    </div>
  </template>
  ...
  export default defineComponent({
    setup() {
      ...
      const checkAnswer = (selected: number) => {
        ...
        isCorrectAnswer.value = selected === quiz.value.correctAnswer;
  
        feedback.value = isCorrectAnswer.value ? '正解！' : '残念・・・外れ';
      };
  
      return { theme, quiz, fetchQuiz, fetchNextQuiz, feedback, selectedAnswer, isCorrectAnswer, checkAnswer };
    },
  });
  </script>
